...
def build_dataset(neg, pos, trans=True, usessa=False):
    ...
    if usessa:
        ...
        for im in nlist:
            ...
            nims.append(np.asarray(r_max).flatten())
        ...
        for im in plist:
            r, pr, r_max = ssa_clustering(im, 40, 2, 0, 0.1, False, 42)
            pims.append(np.asarray(r_max).flatten())
            if len(pims) % 10 == 0:
...
if __name__ == "__main__":
    ...
    dataset, rate = build_dataset(NEGATIVE_DIR, POSITIVE_DIR, usessa=False)
    l = len(dataset)
...
#    lsvc = SVC(C=2,kernel='rbf', class_weight='balanced').fit(X, y)
